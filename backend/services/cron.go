...
func (s *CronService) processUser(user models.User) {
	...
	// 12 AM Tue-Sat: Generate daily summary (for Mon-Fri logs)
	isTueToSat := weekday >= time.Tuesday && weekday <= time.Saturday
	if isTueToSat && hour == 0 && minute < 30 {
		s.generateDailySummary(user)
	}

	// 1 AM Tue-Sat: Auto-retry fallback summaries (if any exist)
...
func (s *CronService) retryFallbackSummaries(user models.User) {
	...
	log.Printf("Found %d fallback summaries to regenerate at 1 AM for user %s", len(fallbackSummaries), user.Email)

	for _, summary := range fallbackSummaries {
		log.Printf("Attempting automated 1 AM fallback regeneration for %s (date: %s)...", user.Email, summary.LogDate.Format("2006-01-02"))

		generatedSummary, isFallback := s.geminiService.GenerateDailySummary(summary.RawLogs)
		if isFallback {
			log.Printf("Automated 1 AM fallback regeneration failed again for %s (date: %s)", user.Email, summary.LogDate.Format("2006-01-02"))
			continue
		}
		...
		if err := db.DB.Save(&summary).Error; err != nil {
			log.Printf("Failed to save regenerated summary at 1 AM for %s: %v", user.Email, err)
		} else {
			log.Printf("Successfully regenerated daily summary at 1 AM for %s (date: %s)", user.Email, summary.LogDate.Format("2006-01-02"))
		}
	}
}
